<x-dashboard-tile :position="$position" refresh-interval="60">
    <div style="height: 100%;">
        <a class="font-small text-dimmed text-xs lowercase tracking-wide tabular-nums absolute top-0 left-0 m-1"
          href="{{ route('fetch-mixed-content') }}">
        ...
        <h1 class="font-medium text-dimmed text-sm uppercase tracking-wide tabular-nums text-center">Mixed Content</h1>
        ...
        <div class="flex flex-row justify-center">
          <div class="flex flex-col w-1/4 border border-gray-700 border-dimmded rounded pt-3 pb-3 bg-gray-700 m-1">
              <div class="font-small text-xs text-center">{{ $secureCount + $insecureCount }}</div>
              <div class="font-small text-xs text-center">All</div>
          </div>
          <div class="flex flex-col w-1/4 border border-gray-700 border-dimmded rounded pt-3 pb-3 bg-green-700 m-1">
              <div class="font-small text-xs text-center">{{ $secureCount }}</div>
              <div class="font-small text-xs text-center">Secure</div>
          </div>
          <div class="flex flex-col w-1/4 border border-gray-700 border-dimmded rounded pt-3 pb-3 bg-red-700 m-1">
              <div class="font-small text-xs text-center">{{ $insecureCount }}</div>
              <div class="font-small text-xs text-center">Insecure</div>
          </div>
        </div>

        <div class="h-56 flex mt-3">
          <ul class="h-48 w-full m-auto overflow-y-scroll">
            <div class="text-center @if($insecureCount <= 0) block @else hidden @endif">No insecure content</div>
            @if( array_key_exists('insecure', $links) )
            @foreach( $links['insecure'] as $link )
            <div class="flex flex-row justify-between py-1 relative">
                <div class="flex flex-col w-full text-xs relative">
                    <div class="p-1 w-full text-left text-xs">-> {{ $link }}</div>
                </div>
            </div>
            @endforeach
            @endif
